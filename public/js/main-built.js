...
define('main',['jquery','Holder'], function ($) {
    require(['bootstrap']);
...
define("vendor/jquery.sparkline.min", function(){});

/*global Icinga:false, document: false, define:false require:false base_url:false console:false */
define('icinga/util/mixin',[],function() {
...
define('logging',['icinga/util/mixin'], function(mixin) {
    ...
    /**
     * Allows objects to inherit the logging capabilities,
     * mainly used in the Icinga object
     * @param obj
     */
    features.enableInterfaceFor = function(obj) {
        mixin(obj,features);
    };
...
(function() {
    ...
    var registeredBehaviours = {};
    var BASEPATH = 'icinga/behaviour/';

    define('icinga/behaviour',['logging','icinga/util/mixin'],function(log,mixin) {

        var CallInterface = function() {
            /**
             * Helper method to determine if a behaviour is currently enabled for the container (or the whole
             * document, if no container is given)
             *
             * @return bool
             */
            this.isActive = function(behaviour,container) {
                return (typeof registeredBehaviours[behaviour] === "object" &&
                    registeredBehaviours[behaviour].isEnabledFor(container));
            };
             ...
             * @example
             *
             * behaviourMgr.registerBehaviour("myCoolTable",function(behaviour) {
             *
             * },log.error);
             *
             * @param name
             ...
             * @param errorCallback
             */
            this.registerBehaviour = function(name,successCallback,errorCallback) {
                if(registeredBehaviours[name])
                    successCallback(registeredBehaviours[name]);
                require([BASEPATH+name],function(behaviour) {
                    log.debug("Loaded behaviour "+name);
                    registeredBehaviours[name] = behaviour;
                    successCallback(behaviour);
                },function(err) {
                    errorCallback("Could not load behaviour "+name+" "+err);
                });
            };

            /**
             * Allows objects to inherit the behaviour capabilities,
             * mainly used in the Icinga object
             * @param obj
             */
            this.enableInterfaceFor = function(obj) {
                mixin(obj,this);
            };
...
define('icinga/widgets/checkIcons',['logging'], function(log) {
    var rad = Math.PI / 180;
...
define('icinga/widgets/subTable',['jquery','logging'], function($,log) {
    return function() {
        this.el;
        this.count = 10;
        this.offset = 0;
...
(function() {
    ...
    var ContainerMgr = function($,log,widgets,subTable,holder) {
        ...
        this.initControlBehaviour = function(root) {
            ...
            $('div[container-id] .text-editor',root).each(function() {

                    var editor = ace.edit(this);
                    editor.setTheme("ace/theme/chrome");
                    editor.getSession().setMode("ace/mode/icinga");
            })
        }
    ...
    define('icinga/container',['jquery','logging','icinga/widgets/checkIcons','icinga/widgets/subTable','Holder'],function($,log,widgets,subTable,holder) {
...
(function() {
    ...
    var AsyncManager = function(containerMgr,log,mixin,$) {
        var pending = {
        ...
        var handleFailure = function(error) {
            ...
            if(this.destination) {
                containerMgr.updateContainer(this.destination,error.responseText,this);
            }
        };


        this.clearPendingRequestsFor = function(destination) {
            if(!$.isArray(pending[destination])) {
                pending[destination] = [];
                return;
            } else {
                for(var i=0;i<pending[destination];i++) {
                    var req = destination[i];
                    req.abort();
                }
                pending[destination] = [];
            }
    ...
    // Register the AsyncManager as a singleton object
    define('icinga/util/async',['icinga/container','logging','icinga/util/mixin','jquery'],function(containerMgr,log,mixin,$) {
        if(typeof asyncMgrInstance !== null)
            asyncMgrInstance = new AsyncManager(containerMgr,log,mixin,$);
        return asyncMgrInstance;
    });
})();
/*global Icinga:false, document: false, define:false require:false base_url:false console:false */
define('icinga/icinga',[
    ...
    'icinga/util/async',
    'icinga/container'
], function ($,sparkline,log,behaviour,async,containerMgr) {
    ...
    var Icinga = function() {
        ...
        var initialize = function () {
            // extend the icinga object with the most important mixins, so they are available
            // via Icinga.*()

            log.enableInterfaceFor(this);
            async.enableInterfaceFor(this);

            this.debug("Setting up behaviours");
            behaviour.enableInterfaceFor(this);
            this.loadDefaultBehaviours();
            this.debug("Behaviours set up");

            containerMgr.initializeContainers(document);
            this.debug("Initialization finished");
        };
        ...
        this.loadDefaultBehaviours = function() {
            this.registerBehaviour("actionTable",enableCb,this.error);
            this.registerBehaviour("mainDetail",enableCb,this.error);
        };
