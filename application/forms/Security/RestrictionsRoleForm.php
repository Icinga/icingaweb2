...
class RestrictionsRoleForm extends ConfigForm
{
    ...
    public function init()
    {
        ...
        foreach ($moduleManager->listInstalledModules() as $moduleName) {
            $module = $moduleManager->getModule($moduleName, false);
            foreach ($module->getProvidedRestrictions() as $restriction) {
                ...
                while (isset($this->providedRestrictions[$moduleName][$name])) {
                ...
                $this->providedRestrictions[$moduleName][$name] = array(
    ...
    {
        foreach ($this->providedRestrictions as $moduleName => $restrictions) {
            // todo add h3 with subsubheader(?) class and ucfirst($moduleName) as content
            foreach ($restrictions as $name => $spec) {
                $this->addElement(
                    'text',
                    $name,
                    array(
