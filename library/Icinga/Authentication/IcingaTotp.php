...
class IcingaTotp
{
    ...
    {
        return sprintf(
            'otpauth://totp/%1$s:%2$s?secret=%3$s&issuer=%1$s&algorithm=%4$s&digits=%5$u&period=%6$u',
            urlencode('icingaweb2'),
            urlencode($this->user),
            urlencode($this->totp->getSecret()),
            urlencode($this->totp->getDigest()),
            urlencode($this->totp->getDigits()),
            urlencode($this->totp->getPeriod())
        );
    }
