...
class TCPDF {
    ...
    public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
        $file = preg_replace('|^.+/nowhere|', '', $file);
		if ($this->state != 2) {
			return;
		}
		if ($x === '') {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		switch($dest) {
			case 'I': {
                ...
                if ($a = ob_get_contents()) {
					$this->Error(var_export($a, 1) . 'aa Some data has already been output, can\'t send PDF file');
				}
				if (php_sapi_name() != 'cli') {
					// send output to a browser
					header('Content-Type: application/pdf');
					if (headers_sent()) {
						$this->Error('bb Some data has already been output to browser, can\'t send PDF file');
					}
			...
			case 'D': {
				...
				if (ob_get_contents()) {
					$this->Error('cc Some data has already been output, can\'t send PDF file');
				}
				header('Content-Description: File Transfer');
				if (headers_sent()) {
					$this->Error('ee Some data has already been output to browser, can\'t send PDF file');
				}
				header('Cache-Control: private, must-revalidate, post-check=0, pre-check=0, max-age=1');
				//header('Cache-Control: public, must-revalidate, max-age=0'); // HTTP/1.1
				header('Pragma: public');
			...
			case 'FD': {
				...
				if ($dest == 'FI') {
					...
					// send headers to browser
					if (ob_get_contents()) {
						$this->Error('ff Some data has already been output, can\'t send PDF file');
					}
					header('Content-Description: File Transfer');
					if (headers_sent()) {
						$this->Error('nn Some data has already been output to browser, can\'t send PDF file');
					}
					header('Cache-Control: private, must-revalidate, post-check=0, pre-check=0, max-age=1');
					header('Pragma: public');
					header('Expires: Sat, 26 Jul 1997 05:00:00 GMT'); // Date in the past
